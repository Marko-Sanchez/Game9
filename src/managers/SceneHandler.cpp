...
namespace Manager
{
SceneHandler::SceneHandler(std::shared_ptr<Renderer::Shader> shader, std::shared_ptr<Core::Window> window, std::shared_ptr<Renderer::Texture2D> texture):
m_renderer(std::make_unique<Renderer::SpriteRenderer>(shader)),
m_window(window),
m_texture(texture)
{
    assert(m_renderer);
